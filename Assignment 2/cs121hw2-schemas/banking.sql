-- [Problem 0]
-- 1) As shown in slide 40 of lecture 3, we know that the float and double doesn't represent 
-- the base 10 properly. In other words, by using either float or double, it might lead to 
-- miscalculation, or the value that is slightly off. If this accumulates, this could lead to 
-- significant bank balance error. 
-- 2) We could represent account_number with a type differnet than VARCHAR, since conventional 
-- account number has fixed lengths and different numeric combinations. Using char will prevent
-- from fragmentation. 
...
-- [Prolem 1b]
-- Retrieve the loan-number and amount of all loans owned by Smith.  Order the results byincreasing loan #.
...
SELECT branch_city FROM branch NATURAL JOIN account WHERE account_number = 'A-446';

    
-- [Problem 1d]
-- Retrieve the customer name, account number, branch name, and balance, of accounts 
-- owned by customers whose names start with “J”.

SELECT customer_name, account_number, branch_name, balance FROM account NATURAL JOIN depositor
...
-- [Problem 2b]
-- Write a SQL query thatreports the name of any customers that have neither an account nor a loan. 
...
-- [Problem 2c]
-- The bank decides to promote its branches located in the city of Horseneck, so it wants to make a 
-- $75 gift-deposit into all accounts held at branches in the city of Horseneck.  
-- Write the SQLUPDATE​ command for performing this operation.
...
SELECT account_number, branch_name, balance FROM account NATURAL JOIN (select branch_name, MAX(balance) AS balance
    FROM account GROUP BY branch_name) AS max;
    
-- [Problem 2f]
-- Implement the same query as in the previous problem, this time using an ​IN​ predicate withmultiple columns.

SELECT account_number, branch_name, balance FROM account WHERE (branch_name, balance) IN 
    (SELECT branch_name, MAX(balance) FROM account GROUP BY branch_name);

-- [Problem 3]
-- Compute the rank of all bank branches, based on the amount of assets that each branch holds.

SELECT branch_name, assets, COUNT(*) AS 'rank' FROM 
    (SELECT a.branch_name, a.assets FROM branch a, branch b WHERE a.assets < b.assets 
